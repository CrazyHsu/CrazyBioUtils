...
#usage information
my $usage = <<USAGE;
grep_psl_by_target V1.0, written by corephi

----------------------------------------------------------
Usage: grep_psl_by_target.pl -i in.psl -l list.txt -o out.psl
 Options:
...
cat in.psl | grep_psl_by_target.pl -i - -l id.list > out.psl
cat id.list | grep_psl_by_target.pl -i in.psl -l - > out.psl
...
while (<$afile_fh>) {
    ...
    if ($reverse_flag) {
        print $outfile_fh $line unless exists $lists{$t_name};
    }
    else {
        print $outfile_fh $line if exists $lists{$t_name};
    }
