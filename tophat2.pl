...
foreach my $basename ( sort keys %fastqs ) {
    ...
    my $command =
"tophat -o $out_folder/$basename $tophat_param $bowtie2db $fq_in_param >$log_folder/$stdout 2> $log_folder/$stderr";
	if ($summarize) {
		warn "\t$command\n";		
	}else{
		warn "\t$command\n";
	...
	# p %counts;
	my $data = $mode eq 'pe' ? 
		"$basename\t$counts{left}{input}\t$counts{left}{multiple}\t$counts{left}{unique}\t$counts{right}{input}\t$counts{right}{multiple}\t$counts{right}{unique}\t$counts{pair}{multiple}\t$counts{pair}{unique}\t$counts{pair}{concordant}\t$counts{pair}{discordant}\t$counts{rate}{overal}\t$counts{rate}{concordant}\n" :
		"$basename\t$counts{pair}{input}\t$counts{pair}{multiple}\t$counts{pair}{unique}\t$counts{rate}{concordant}\n";
    print $data;
;
}
