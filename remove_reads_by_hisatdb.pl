...
my $usage = <<USAGE;
SYSNOPSIS
RemoveReadsByhisatdb.pl [options] bw2db file|glob
...
-e "$xRNA_bowtiedb.1.bt2"
  ...
  || die "Cannot detect hisat db:$xRNA_bowtiedb";
my $glob = shift;
my @fqs  = glob "$glob";
die $usage unless @fqs;
can_run('hisat') or die 'hisat is not installed!';
$out_folder =~ s/[\/|\|]+$//;
...
my $hisat_dexRNA_param = "-k 1 -t -p $progress";
my $fq_in_param          = "";
my $fq_out_param         = "";
...
foreach my $basename ( sort keys %fastqs ) {
    ...
    my $command =
"hisat $hisat_dexRNA_param -x $xRNA_bowtiedb $fq_in_param $fq_out_param >/dev/null 2> $log_filename";
    warn "\t$command\n";
    ...
    my %counts = summarize_hisat( $log_filename, $mode );
    print
