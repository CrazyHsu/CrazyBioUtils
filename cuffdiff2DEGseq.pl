...
GroupName List is equal to cuffdiff's '-L' option, if ommited cuffdiff2DEGseq will
automatically calculated one.
If all the group have only one replicate, this program will restore swich to 
DEGseq's 'blind' and 'fit-only' way to estimate dispersions
...
for ( my $i = 0 ; $i < $#groups ; $i++ ) {
    for ( my $j = 1 ; $j <= $#groups ; $j++ ) {
        ...
        if ( $group1_reps == 1 && $group2_reps == 1 ) {
            DEGseq_without_replicate();
        }
        else {
            warn "group has more than one replicates\n";
        }
...
sub DEGseq_without_replicate {
    my $Rocode = <<DEGseq;
library("DEGseq")
print("R version:")
version
print("DEGseq version:")
packageVersion("DEGseq")
data <- read.table(file="total.count",header=TRUE)
head(data)
Design <- read.table(file="design.txt",header=TRUE,row.names = 1)
conds <- levels(Design[, 1])
condA <- conds[2]
print(condA)
condB <- conds[1]
print(condB)
DEGexp(geneExpMatrix1=data, geneCol1=1, expCol1=3, groupLabel1=condA,geneExpMatrix2=data, geneCol2=1, expCol2=2, groupLabel2=condB,method="MARS",outputDir='./',qValue=0.01,thresholdKind=4)
DEGseq

    open my $log_fh, ">", "DEGseq.log" or die "cannot create data file\n";
    my $R     = Statistics::R->new();
