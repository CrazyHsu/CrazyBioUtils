...
my $stdout = "$out_folder.stdout.log";
my $stderr = "$out_folder.stderr.log";
my $command =
"tophat -o $out_folder $tophat_param $bowtie2db $fq_in_param >$log_folder/$stdout 2> $log_folder/$stderr";
if ($summarize) {
	warn "\t$command\n";		
}else{
	warn "\t$command\n";
...
my %counts = summarize_tophat2( "$out_folder/align_summary.txt", $mode );
# p %counts;
my $data = $mode eq 'pe' ? 
	"$out_folder\t$counts{left}{input}\t$counts{left}{multiple}\t$counts{left}{unique}\t$counts{right}{input}\t$counts{right}{multiple}\t$counts{right}{unique}\t$counts{pair}{multiple}\t$counts{pair}{unique}\t$counts{pair}{concordant}\t$counts{pair}{discordant}\t$counts{rate}{overal}\t$counts{rate}{concordant}\n" :
	"$out_folder\t$counts{pair}{input}\t$counts{pair}{multiple}\t$counts{pair}{unique}\t$counts{rate}{concordant}\n";
print $data;
