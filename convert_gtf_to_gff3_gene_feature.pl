...
my $usage = <<USAGE;
SYSNOPSIS
fix_gtf.pl transcript1.gtf transcript2.gtf...
USAGE

my @gtf_files = @ARGV;
die "no gtf files\n$usage\n" unless @ARGV;
foreach my $gtf_file (@gtf_files) {
	die "$gtf_file does not exists\n" unless -e $gtf_file
}
my @suffix_lists = qw(.gtf .gff);
...
foreach my $gtf_file (@gtf_files) {
	...
	my ($basename, $path, $suffix) = fileparse($gtf_file, @suffix_lists);

	#############################################################################
	#writing the New GTF
	#############################################################################
	open my $out_fh, ">", $basename."_fixed"."gff";
	...
	foreach my $gid (keys %genes) {
		...
		foreach my $tid (keys %{$genes{$gid}}) {
			$seqname = $genes{$gid}{$tid}{seqname};
			$source = $genes{$gid}{$tid}{source};
			$score = $genes{$gid}{$tid}{score};
			$strand = $genes{$gid}{$tid}{strand};
			push @starts,$genes{$gid}{$tid}{start};
			push @ends,$genes{$gid}{$tid}{end};
		};
